...
//        try {
//            // 提交异步任务
//            Future<?> futureUsers = executorService.submit(() -> importUsers(userRecords));
//            Future<?> futureVideos = executorService.submit(() -> importVideos(videoRecords));
//            Future<?> futureDanmu = executorService.submit(() -> importDanmu(danmuRecords));
//
//            // 等待任务完成
//            futureUsers.get();
//            futureVideos.get();
...
//            log.error("Error occurred during data import", e);
...
//
//
//    private void importVideos( List<VideoRecord> videoRecords) {
//        try(Connection conn = dataSource.getConnection()) {
//            conn.setAutoCommit(false);
//            //videos
//            String videoSql = "INSERT INTO videos (BV, title, owner_Mid, commit_time, review_time, public_time, duration, description, reviewer) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
